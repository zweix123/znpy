...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def init_strategy_settings(self) -> None:
        ...
        for class_name in self.class_names:
            setting: dict = self.backtester_engine.get_default_setting(class_name)  # noqa
    ...
    def init_ui(self) -> None:
        ...
        self.symbol_line: QtWidgets.QLineEdit = QtWidgets.QLineEdit("")  # noqa
        ...
        self.end_date_edit: QtWidgets.QDateEdit = QtWidgets.QDateEdit(QtCore.QDate.currentDate())  # noqa
        ...
        backtesting_button: QtWidgets.QPushButton = QtWidgets.QPushButton("开始回测")  # noqa
        backtesting_button.clicked.connect(self.start_backtesting)

        optimization_button: QtWidgets.QPushButton = QtWidgets.QPushButton("参数优化")  # noqa
        optimization_button.clicked.connect(self.start_optimization)

        self.result_button: QtWidgets.QPushButton = QtWidgets.QPushButton("优化结果")  # noqa
        self.result_button.clicked.connect(self.show_optimization_result)
        self.result_button.setEnabled(False)

        downloading_button: QtWidgets.QPushButton = QtWidgets.QPushButton("下载数据")  # noqa
        downloading_button.clicked.connect(self.start_downloading)

        self.order_button: QtWidgets.QPushButton = QtWidgets.QPushButton("委托记录")  # noqa
        self.order_button.clicked.connect(self.show_backtesting_orders)
        self.order_button.setEnabled(False)

        self.trade_button: QtWidgets.QPushButton = QtWidgets.QPushButton("成交记录")  # noqa
        self.trade_button.clicked.connect(self.show_backtesting_trades)
        self.trade_button.setEnabled(False)

        self.daily_button: QtWidgets.QPushButton = QtWidgets.QPushButton("每日盈亏")  # noqa
        self.daily_button.clicked.connect(self.show_daily_results)
        self.daily_button.setEnabled(False)

        self.candle_button: QtWidgets.QPushButton = QtWidgets.QPushButton("K线图表")  # noqa
    ...
    def load_backtesting_setting(self) -> None:
        ...
        if start_str:
            start_dt: QtCore.QDate = QtCore.QDate.fromString(start_str, "yyyy-MM-dd")  # noqa
    ...
    def start_backtesting(self) -> None:
        ...
        dialog: BacktestingSettingEditor = BacktestingSettingEditor(class_name, old_setting)  # noqa
        i: int = dialog.exec()
    ...
    def start_optimization(self) -> None:
        ...
        dialog: OptimizationSettingEditor = OptimizationSettingEditor(class_name, parameters)  # noqa
        i: int = dialog.exec()
        if i != dialog.Accepted:
            return
    ...
    def edit_strategy_code(self) -> None:
        ...
        file_path: str = self.backtester_engine.get_strategy_class_file(class_name)  # noqa
...
class OptimizationResultMonitor(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        ...
        for n, tp in enumerate(self.result_values):
            ...
            setting_cell: QtWidgets.QTableWidgetItem = QtWidgets.QTableWidgetItem(str(setting))  # noqa
            target_cell: QtWidgets.QTableWidgetItem = QtWidgets.QTableWidgetItem(f"{target_value:.2f}")  # noqa
...
class BacktestingResultDialog(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        ...
        self.table: QtWidgets.QTableWidget = self.table_class(self.main_engine, self.event_engine)  # noqa
...
class CandleChartDialog(QtWidgets.QDialog):
    ...
    def update_trades(self, trades: list) -> None:
        ...
        for d in trade_pairs:
            ...
            pen: QtGui.QPen = pg.mkPen(color, width=1.5, style=QtCore.Qt.DashLine)  # noqa
            ...
            open_text: pg.TextItem = pg.TextItem(f"[{volume}]", color=text_color, anchor=(0.5, 0.5))  # noqa
            close_text: pg.TextItem = pg.TextItem(f"[{volume}]", color=text_color, anchor=(0.5, 0.5))  # noqa

            open_text.setPos(open_ix, open_y - open_side * y_adjustment * 3)
            close_text.setPos(close_ix, close_y - close_side * y_adjustment * 3)  # noqa
